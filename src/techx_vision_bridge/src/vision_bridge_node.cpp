    ...
    : Node("vision_bridge_node")
{
    ...
    // ── 看门狗定时器（2Hz，仅检查是否失联，不触碰网卡，给控制节点让 CPU） ──
        ...
        if (n < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                ...
                if (elapsed > _reconnect_timeout_sec && initialized) {
                    RCLCPP_WARN(this->get_logger(),
                        "UDP 接收超时 %.1fs > 重连阈值 %.1fs，尝试重连...",
                        elapsed, _reconnect_timeout_sec);
                    tryReconnect();
                }
                continue;
            }
...
void VisionBridgeNode::processLoop() {
    ...
    while (!_stop.load()) {
        ...
        RCLCPP_DEBUG(this->get_logger(),
            "发布目标: Seq=%u TID=%u Pos=(%.3f, %.3f, %.3f) T=%.6f",
            target.seq, target.track_id, target.x, target.y, target.z, target.timestamp);
    }
